...
class protoJobManager {
    ...
    start(io) {
        ...
        this.io.sockets.on('connection', function(socket) {
            ...
            // 创建变量
            socket.on('createVariable', (data) => {
                // data -> {routeIndex:, varName:, varDesc:}
                if (commonJs.isUndefinedOrNull(data.routeIndex)) {
                    checkMsgResult(new Error('missing param'), '参数格式不对');
                    return;
                }
                ...
                if (commonJs.isUndefinedOrNull(data.varName)) {
                    checkMsgResult(new Error('missing param'), '不能定义空变量');
                    return;
                }

                // 绑定指令
                ins.setBindVariable({name:data.varName, desc:data.varDesc});
            ...
            // 绑定设定变量
            socket.on('setBindVariable', (data) => {
                // data -> {routeIndex:, varName:, varDesc:}
                if (commonJs.isUndefinedOrNull(data.routeIndex)) {
                    checkMsgResult(new Error('missing param'), '参数格式不对');
                    return;
                ...
                let script = null;
                if (data) {
                    script = ins.setBindVariable({name:data.varName, desc:data.varDesc});
                } else {
                    // 删除变量
                    script = ins.setBindVariable({});
                } 
