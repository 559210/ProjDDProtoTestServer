...
exp.removeJob = function(jobId, callback) {
    ...
    async.series(
        [
            ...
            (cb) => {
                connection.query("insert into ProtoRunnerJobsDel(jobId, jobJson, createTime, modifyTime) select jobId, jobJson, createTime, modifyTime from ProtoRunnerJobs where jobId=?", [jobId], cb);
            },
            (cb) => {
...
///////////////////////////// 取协议流程
exp.loadUserSessionList = function(page, pageSize, callback) {
    if (page === -1) {
        ...
        protocolPool.query("call sp_getRecordByPage ('t_recordProtocol_tools','id','distinct `sessionId`, `userId`',"+pageSize+","+page+",@a,@b,' where length(`userId`)> 0',' ORDER BY `beforTime`',0)", (err, results,fields) => {
            var dataCount = results[1][0].DataCount;
            var pageCount = results[2][0].PageCount;
            callback(err, results[0], dataCount, pageCount);
        });
    }
};
